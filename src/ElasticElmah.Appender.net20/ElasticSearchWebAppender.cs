    ...
    public class ElasticSearchWebAppender : ElasticSearchAppender
    {
        ...
        private static Dictionary<string, object> MapToJson(Dictionary<string, object> dictionary)
        {
			...
			foreach (var kv in dictionary) {
				result.Add (JsonNameConvention (kv.Key),kv.Value);
			}
			return result;
        }
        ...
        {
			var split= (key ?? string.Empty).ToLower().Split('_');
			var result = new string[split.Length];
			for (var i = 0; i < split.Length; i++) {
				result [i] = FirstLetterToUpper (split[i]);
			}
        ...
        {
            if (val.Length > 0)
                return val[0].ToString().ToUpper() + (val.Length > 1 ? val.Substring(1) : string.Empty);
            return string.Empty;
        }
