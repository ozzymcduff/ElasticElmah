...
namespace ElasticElmah.Appender.Presentation
{
    public class LexStackTrace
    {
		...
		bool Contains (char[] characters, char lookingFor)
		{
			foreach (var expected in characters) {
				if (lookingFor == expected)
					return true;
			}
			return false;
		}
		bool Contains(string str,char lookingFor)
		{
        ...
        void CheckForWhiteSpace(string str, int index, Symbols type)
        {
            if (Position < index)
            {
                ...
                {
                    if (!Contains(_whitespaces,str[i]))
                    {
...
{3}", 
        ...
        {
			if (Contains(_str.Substring(index,length),'/'))
            {
...
");
		...
		static IEnumerable<Token> FilterWhitespace (List<Token> _tokens)
		{
			var list = new List<Token> ();
			foreach (var item in _tokens) {
				if (item.Type != Symbols.Whitespace)
					list.Add (item);
			}
			return list.ToArray ();
		}
        ...
        public static IEnumerable<Token> Tokenize(string str)
        {
			...
			return FilterWhitespace( transformer._tokens)
                ;
        }
        ...
        public void TokenizeLines()
        {
            ...
            {
				if (Contains(chars,_str[j]) && i + 1 < j)
                {
                    TokenizeLine(i, j - i);
                    i = j;
                }
